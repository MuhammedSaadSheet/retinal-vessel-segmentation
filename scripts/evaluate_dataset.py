...
def evaluate_segmentation(pred_mask, gt_mask, fov_mask=None):
    ...
    # If a field-of-view mask is provided, evaluate only inside the retina.
    # This avoids evaluating irrelevant black background outside the circular image.
    ...
    # Confusion matrix:
    # tn = true negatives  -> background correctly detected as background
    # fp = false positives -> background wrongly detected as vessel
    # fn = false negatives -> vessel wrongly missed
    # tp = true positives  -> vessel correctly detected as vessel
